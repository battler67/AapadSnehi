...
| Public disaster assistance leads | Server-side Bluesky search with `atproto`; optional hosted Twitter-RoBERTa sentiment | Bounded, on-demand discovery of explicit offers, active aid, institutional support, and fundraising/donation leads with advisory tone labels | Unverified social lead only; neither deterministic confidence nor model sentiment proves identity, availability, capability, proximity, or consent. Results are not persisted. |
